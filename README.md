...
다음은 단어를 추출하는 예시이다.
사용 기술: Python의 자연어 처리 라이브러리 Komoran 이
용, LDA 알고리즘
...
위에서 추출된 단어들은 (동물, 반려, 서비스, 카페, 물건, 로봇, 반려, 커피, 배달)이다.
그다음 제목을 이용하여 뽑은 태그들과의 중복을 제외한 후 총 10개의 최종 내용 태그를 뽑았다.
만약 제목에서 3개의 태그를 추출하였다면

문서의 내용으로 추출한 태그들로 나머지 7개를 채워 총 10개의 내용 태그를 추출했다.
